...
def ionrecAnalysis(Z=5, finalChargeState=3, elementSymbol='B', monteCarloLength=100, numCIModels=1, numEAModels=1, 
  ...
  for mcInd in range(1,monteCarloLength+1):
    ...
    csExp = np.zeros(len(xs))
    totalCS.append(crossSecModel.eval(params=resultMinimizer.params, electronEnergy=xs))
    if makePlots: 
      plt.figure()
...
xs, urdamCS, xsFit, origCS, origUnc, expCS, monteData,Tlist, expRatesNonNeg, (avgExpRates, lowerExpConf, upperExpConf), avgExpRatesUnc, totalUrdRates = ionrecAnalysis(Z=Z, finalChargeState=finalChargeState, elementSymbol=elementSymbol, monteCarloLength=monteCarloLen, numCIModels=1, numEAModels=1, 
                  lowTempPower=4, highTempPower=9, numTempSteps=300, makePlots=False, makePlotsRates=True, FixD=True, addZeroPt=True)

fig, ax = plt.subplots(1,1)

ax.errorbar(xs, origCS, origUnc, color='black', linestyle='none', marker='x', label=f'{elementSymbol}$^{finalChargeState-1}$$^+$ Experiment', zorder=99)
ax.plot(0,0,color='black', marker='o', linestyle='none', label='Monte Carlo data')
ax.plot(0,0,color='black', marker='none', linestyle='-', label='Monte Carlo fit')
...
ax.plot([.074,.074],[0,1],color='blue', label='Ionization threshold', alpha=0.4, linewidth=2)
ax.legend(loc='lower right')
manager = plt.get_current_fig_manager()
manager.window.showMaximized()
ax.set_xlabel('Electron energy (keV)')
ax.set_ylabel('EII Cross section (cm$^2$)')
ax.set_xlim(.05, .14)
ax.set_ylim(5.5e-19,2.9e-17)

ax.set_yscale('log')
fig.tight_layout()

import matplotlib.backends.backend_pdf
pdf = matplotlib.backends.backend_pdf.PdfPages(f"crossMonteFe{monteCarloLen}.pdf")
plt.tight_layout()
pdf.savefig( fig )
pdf.close()
