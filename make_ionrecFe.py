...
def ionrecAnalysis(Z=5, finalChargeState=3, elementSymbol='B', monteCarloLength=100, numCIModels=1, numEAModels=1, 
  ...
  for mcInd in range(1,monteCarloLength+1):
    ...
    EAprefixes = []
    LowestEion = params['ci'][-1][1] #lowest ionization energy
    ...
    ### sometimes EA models are not in order in params['ea']
    ### here we sort them with the lowest energy ones (highest amplitude) first
    eaEIons = [[p[1],p[0]] for p in params['ea']] #energy and number for each curve from atomdb
    sortedeaEIindexes = [int(ea[1])-1 for ea in sorted(eaEIons)] #indexes of params['ea'] to use in order of lowest energy
    
    #ipdb.set_trace()
    for i in sortedeaEIindexes[:numEAModels]:
      prefix = f'ea{i+1}_'
...
if __name__ == '__main__':
  ...
  if False:
    ...
    for i, chargeState in enumerate(ionrecArgs['finalChargeState']):
      searchstringexp = os.path.join(os.path.dirname(__file__), DATADIR,element.upper(),"SI",f'{element.upper()}{int(chargeState-1)}+.xlsx')
      try:
        fname = glob.glob(searchstringexp)[0]
        tmpArgsDict = {}
        ...
        print(f"Skipped {element}{int(chargeState-1)}+ (missing file).")
    import matplotlib.backends.backend_pdf
    pdf = matplotlib.backends.backend_pdf.PdfPages("IronOutput.pdf")
