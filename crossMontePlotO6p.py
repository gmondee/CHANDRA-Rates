...
ax.errorbar(xs, origCS, origUnc, color='black', linestyle='none', marker='x', label=f'O$^{finalChargeState-1}$$^+$ Experiment', zorder=99)
...
ax.set_xlim(.7, 8.4)
ax.set_ylim(9e-21,1.1e-19)
ax.set_yscale('log')
fig.tight_layout()

import matplotlib.backends.backend_pdf
pdf = matplotlib.backends.backend_pdf.PdfPages(f"crossMonte{monteCarloLen}.pdf")
plt.tight_layout()
pdf.savefig( fig )
pdf.close()
